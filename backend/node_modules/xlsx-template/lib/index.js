...
var Workbook = /** @class */ (function () {
    ...
    Workbook.prototype.copySheet = function (sheetName, copyName, binary) {
        ...
        var finalSheetName = copyName || 'Sheet' + newSheetIndex;
    ...
    Workbook.prototype.generateUUID = function () {
    ...
    Workbook.prototype.substituteImageInCell = function (cell, substitution) {
        ...
        this.initRichData();
        var maxFildId = this.findMaxFileId(/xl\/media\/image\d*\..*/, /image(\d*)\./);
        /**
        ...
        var futureMetadata = this.metadata.findall('futureMetadata').find(function (fm) {
            return fm.attrib.name === 'XLRICHVALUE';
        });
        var futureMetadataCount = futureMetadata.get('count');
        futureMetadata.set('count', parseInt(futureMetadataCount) + 1);
        var bk = etree.SubElement(futureMetadata, 'bk');
        ...
        var XLRICHVALUEMetaDataTypeIndex = this.metadata.find('metadataTypes').findall('metadataType').findIndex(function (el) {
    ...
    */
    Workbook.prototype.hideCols = function (sheetName, hideItemIndexes) {
        var self = this;
        var sheet = self.loadSheet(sheetName);
        self.sheet = sheet;
        if (Array.isArray(hideItemIndexes) && hideItemIndexes.length) {
            var cols_1 = sheet.root.find("cols");
            if (cols_1) {
                hideItemIndexes.forEach(function (hideIndex) {
                    var colIndex = hideIndex + 1;
                    var col = cols_1.findall("col").find(function (c) {
                        var min = parseInt(c.attrib.min, 10);
                        var max = parseInt(c.attrib.max, 10);
